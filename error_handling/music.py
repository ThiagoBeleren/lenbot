...
class Music(commands.Cog):
    ...
    async def music_embed(self, interaction: nextcord.Interaction, search, title):
        embed = nextcord.Embed(title=f"{title} 🎶", description=f"{search.title}", url=f"{search.uri}")
        embed.add_field(name="Author", value=f"{search.author}")
        embed.add_field(name="Duração", value=f"`{round(search.length / 60, 2)}m`")
        embed.set_footer(text=f"adicionado por {interaction.user.display_avatar}")  # icon_url=ctx.author.display_avatar)
        embed.set_image(url=f"{search.thumb}")
        await interaction.response.send_message(embed=embed)

    async def music_buttons(self, ctx):
        back_btn = Button(style=nextcord.ButtonStyle.gray,
                          emoji="⏪",
                          disabled=True,
                          )
        pause_btn = Button(style=nextcord.ButtonStyle.gray,
                           emoji="⏯️",
                           )
        skip_btn = Button(style=nextcord.ButtonStyle.gray,
                          emoji="⏩",
                          )
        shuffle_btn = Button(style=nextcord.ButtonStyle.secondary,
                             emoji="🔀",
                             disabled=False,
                             )
        stop_btn = Button(style=nextcord.ButtonStyle.red,
                          emoji="⏹️",
                          )
        repeat_btn = Button(style=nextcord.ButtonStyle.secondary,
                            emoji="🔁",
                            disabled=False,
                            )
        ...
        async def backBTN(interaction: nextcord.Interaction):
            return

        async def pauseBTN(interaction: nextcord.Interaction):
            await self.pause_add(ctx)

        pause_btn.callback = pauseBTN

        async def skipBTN(interaction: nextcord.Interaction):
            await self.skip_add(ctx)

        skip_btn.callback = skipBTN

        async def shuffleBTN(interaction: nextcord.Interaction):
            await self.shuffle_add(ctx)

        shuffle_btn.callback = shuffleBTN

        async def stopBTN(interaction: nextcord.Interaction):
            await self.stop_add(ctx)

        stop_btn.callback = stopBTN

        async def repeatBTN(interaction: nextcord.Interaction):
    ...
    @nextcord.slash_command(description="plays a song (youtube)")
    ...
    @nextcord.slash_command(description="Pauses the song")
    async def pause(self, ctx: commands.Context):
        await self.pause_add(ctx)

    @nextcord.slash_command(description="Stops the song")
    async def stop(self, ctx: commands.Context):
        await self.stop_add(ctx)

    @nextcord.slash_command(description="Skips the song")
    async def skip(self, ctx: commands.Context):
        await self.skip_add(ctx)

    @nextcord.slash_command(description="Shuffle a playlist")
    async def shuffle(self, ctx: commands.Context):
        await self.shuffle_add(ctx)

    @nextcord.slash_command(description="Shows the queue")
    async def queue(self, ctx: commands.Context):
        await self.queue_add(ctx)

    @nextcord.slash_command(description="Repeats the current song of the queue")
    async def repeat(self, ctx: commands.Context):
        await self.repeat_add(ctx)

    @nextcord.slash_command(description="Adds your youtube playlist")
