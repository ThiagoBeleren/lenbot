...
class userModeration(commands.Cog):
    ...
    async def addban(self, ctx, member: discord.Member, reason: str):
        ...
        db = sqlite3.connect("userbans.sqlite3")
        cursor = db.cursor()
    ...
    async def removewarn(seld, ctx, member: discord.Member):
        db = sqlite3.connect("userinfo.sqlite")
        cursor = db.cursor()
        cursor.execute("SELECT * FROM userwarnings WHERE user = ? AND guild = ?", (member.id, ctx.guild.id))
    ...
    async def removewarnuser(self, ctx, member: discord.Member):
