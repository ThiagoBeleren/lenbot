...
class Levelsystem(commands.Cog):
        ...
        if not message.content.startswith("/"):
            ...
            if not message.author.bot:
                ...
                if str(message.author.id) in data:
                    ...
                    if new_level > lvl:
                        await message.channel.send(f"{message.author.mention} Aumentou seu Nivel para **{new_level}** :partying_face: !!!")
                        ...
                        for i in range(len(level)):
                            if new_level == level_num[i]:
                                ...
                                mbed = discord.Embed(title=f"{message.author} Voce conseguiu o cargo **{level[i]}**!",
    ...
    async def rank(self, ctx: commands.Context, user: discord.Member):
        ...
        with open("levels.json", "r") as f:
            ...
            ## Rank card
            background = Editor(f"image1.png")
    ...
    async def servidorrank(self, ctx, range_num=10):
        ...
        mbed = discord.Embed(
            title="Tabela de rank do servidor"
        )
    ...
    async def servidorresetarrank(self, ctx, user: discord.Member):
        ...
        if not member == ctx.author:
            ...
            if not role in member.roles:
                await ctx.send(f"Voce so pode resetar seu rank, somente {role.mention} podem alterar ranks de outros membros")#sunglasses emoji
                return
    ...
    async def servidoraumentarnivel(self, ctx, increase_by: int, user: discord.Member):
        member = user or ctx.author
        ...
        await ctx.send(f"{member.mention}, Parabens , seu nivel aumentou para **{increase_by}**")#birthday emoji

    @commands.slash_command(description="aumentar xp")
    @commands.has_role("Moderador")
    async def servidoraumentarxp(self, ctx, increase_by: int, user: discord.Member):
        member = user or ctx.author
        ...
        await ctx.send(f"{member.mention}, Seu xp foi alterado para `{increase_by}`")
...
def setup(bot):
    bot.add_cog(Levelsystem(bot))
        ...
        # #buttons
        # level = Button(style=ButtonStyle.green, label="Level", id="level")
        #
        # guild = self.bot.get_guild(ctx.guild)
        #
        # #embeds
        # levelembed = discord.Embed(title="Seu level neste servidor")
        # levelembed.set_image(url="")
        #
        # await ctx.send(
